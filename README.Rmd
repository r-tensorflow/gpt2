...
Followed by installing the GPT-2 runtime, we recommend usind MiniConda which you will need to install and then restart R session:
...
To generate text using GPT-2, specify a `prompt` and let the mode run,
...
You can use [cloudml](https://github.com/rstudio/cloudml) to run this model in Google Cloud as follows. First, create a `cloudml.yml` file:

```yml
customCommands:
  - ["pip3", "install", "tensorflow>=2.0", "fire", "regex", "requests", "tqdm"]
...
Followed by the following `train.R` script:
...
writeLines(result, "gpt2-result.txt")
```
